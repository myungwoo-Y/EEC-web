...
const Table = React.forwardRef(({ className, children }: PropsWithChildren<TableProps>, ref: React.ForwardedRef<HTMLTableElement>) => {
  return <table ref={ref} className={classNames('w-full', className)}>{children}</table>;
});
...
const UserResultTable = React.forwardRef(
  ({ users, setUsers }: UserResultTableProps, ref: React.ForwardedRef<HTMLTableElement>) => {
    ...
    return (
      <TableContainer className="mt-10 max-h-[800px]">
        <Table ref={ref} className="min-w-[1700px]">
          <THead>
            <Tr>
...
UserResultTable.displayName = 'UserResultTable';
